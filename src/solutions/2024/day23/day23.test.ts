...
import { solveStar1, solveStar2 } from "./day23";
const DAY = parseInt(
...
describe("Day " + DAY, () => {
  it("should pass test", () => {
    expect(solveStar1(testInput)).toBe(7);
  });

  it("should solve star 1", () => {
    expect(solveStar1(input)).toBe(1308);
  });

  it("should pass test for star 2", () => {
    expect(solveStar2(testInput)).toBe("co,de,ka,ta");
  });

  it("should solve star 2", () => {
    expect(solveStar2(input)).toBe("bu,fq,fz,pn,rr,st,sv,tr,un,uy,zf,zi,zy");
  });
});
